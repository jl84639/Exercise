...
price=β0 +β1×livingArea+β2×bathrooms+β3×age^2+β4×(livingArea×bathrooms)+...+ϵ, where "..." includes all the other variables present in the dataset as part of the initial full model. For the linear model, we applied feature engineering to create polynomial terms and interactions, notably a squared term for the age of the houses and an interaction term between living area and bathrooms.
...
plot(hotel_cv$y_hat, hotel_cv$y, main = "Actual & Expected number of bookings With Children",
     xlab = "Expected number of bookings With Children", 
     ylab = "Actual number of bookings With Children", 
     pch = 19,
...
In this sector,our goal is to build a model that can accurately predict the likelihood of a mushroom being poisonous based on various descriptive features. 

Since these features are categorical (such as odor or bruises), they don't have a natural numerical representation that a mathematical model could process. To overcome this, we transform these categories into a format that the model can understand using a technique called one-hot encoding. 

In this specific case, we've removed the 'veil.type' feature before training our model. This decision was made because every mushroom in our dataset has the same 'veil.type' value, meaning this feature doesn't vary and hence cannot provide any useful information for distinguishing between poisonous and non-poisonous mushrooms.

Once we've numerically encoded our categorical data and deleted unnecessary data, we employ a statistical method known as lasso-penalized logistic regression which is suitable for binary outcomes like 'poisonous' vs 'non-poisonous', and the 'lasso' part refers to a penalty applied to the model that has the effect of streamlining the final model.

After changing the category features into numbers and picking out the important ones with the lasso method, we get a better model that can spot the key signs of a poisonous mushroom. The generated plot is as followed:
...
From the confusion matrix below we can see that there are no cases where edible mushrooms were incorrectly predicted as poisonous (False Positives), as indicated by the zero in the e row and 1 column.There are 2 cases where poisonous mushrooms were incorrectly predicted as edible (False Negatives).

In a word,the True Positive Rate (Sensitivity or Recall) is approximately 99.74%, indicating that nearly all poisonous mushrooms were correctly identified.

```{r 4-5, echo=FALSE, warning=FALSE}

#Comparasion of TPR and FPR
...
In summary, the model demonstrates high accuracy and specificity. However, the ultimate assessment of whether this is a good model also depends on the cost of misclassification, its generalizability to unseen data, and whether it meets the specific needs of its application. If the stakes are high, such as in health or safety applications, even a small number of false negatives may be significant, and further investigation into those cases would be warranted.
